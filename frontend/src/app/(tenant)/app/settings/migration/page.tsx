...
export default function DataMigrationHubPage() {
  ...
  const downloadProductsTemplate = () => {
    const csvContent =
      "ProductName,SKU,Category,Packing,HSN,TaxRate,MRP,PurchaseRate,RetailRate,WholesaleRate,BatchNumber,ExpiryDate,OpeningStock\n" +
      "Dolo 650mg Strip 15s,DOL-650,Tablets,15s,30049099,12,30.50,21.00,28.00,25.00,BT-2026-881,12/28,200\n" +
      "Augmentin 625 Duo,AUG-625,Antibiotics,10x10,30049099,12,201.00,145.00,185.00,168.00,AG-99120,08/27,50\n" +
      "Pantocid 40mg Tab,PAN-040,Gastro,10s,30049099,12,140.00,95.00,128.00,115.00,PT-44211,04/28,120\n" +
      "Betadine 10% Ointment 20g,BET-010,Antiseptic,20g,30049099,18,85.00,60.00,78.00,70.00,BT-77219,09/27,40\n";
  ...
  const handleProductsFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Check if row has 13 columns (new format with wholesale) or 12 columns (legacy format)
        let retailRate = 0;
        let wholesaleRate = 0;
        let batch = "";
        let expiry = "";
        ...
        if (row.length >= 13) {
          retailRate = parseFloat(row[8]) || mrp * 0.9;
          wholesaleRate = parseFloat(row[9]) || retailRate;
          batch = row[10] || `BT-${new Date().getFullYear().toString().slice(2)}${String(new Date().getMonth() + 1).padStart(2, "0")}`;
          expiry = row[11] || "12/28";
          openingStock = parseFloat(row[12]) || 0;
        } else {
          retailRate = parseFloat(row[8]) || mrp * 0.9;
          wholesaleRate = retailRate;
          batch = row[9] || `BT-${new Date().getFullYear().toString().slice(2)}${String(new Date().getMonth() + 1).padStart(2, "0")}`;
          expiry = row[10] || "12/28";
          openingStock = parseFloat(row[11]) || 0;
        }
    ...
    for (let i = 0; i < productRows.length; i++) {
      ...
      try {
        await inventoryService.createItem({
          ...
          minimumSellingPrice: row.wholesaleRate || row.sellingPrice,
          mrp: row.mrp,
          minimumStockAlert: 5,
          trackBatches: true,
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-emerald-50/90 via-slate-50 to-orange-50/70 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900 border border-emerald-200 dark:border-slate-800 rounded-2xl p-6 relative overflow-hidden shadow-xs text-slate-900 dark:text-white">
        <div className="absolute -right-10 -bottom-10 opacity-10 pointer-events-none">
          <Database className="w-80 h-80 text-emerald-600 dark:text-emerald-400" />
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 text-xs mt-1 max-w-2xl font-medium">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              ...
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition flex items-center gap-1.5 shadow-md shadow-emerald-900/20 cursor-pointer active:scale-95"
            >
            ...
            <Link
              ...
              className="px-3.5 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-md shadow-orange-900/20"
            >
            ...
            <Link
              href="/app/dashboard"
              className="px-3.5 py-2 bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold transition shadow-xs"
            >
              Dashboard
            </Link>
          </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center gap-2">
          <span className="text-xs font-bold text-slate-700 dark:text-slate-400 mr-2">Migrating From:</span>
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                selectedSoftware === sw.id
                  ...
                  : "bg-white hover:bg-slate-100 text-slate-700 dark:bg-slate-950 dark:text-slate-400 dark:hover:text-white border border-slate-300 dark:border-slate-800 shadow-2xs"
              }`}
            >
              {sw.label}
            </button>
      ...
      <div className="flex space-x-2 border-b border-slate-200 dark:border-slate-800 pb-2">
        ...
        >
          ...
          {partyRows.length > 0 && (
            <span className="px-1.5 py-0.5 rounded-full bg-orange-950 text-[10px] text-orange-300">
              {partyRows.length}
            </span>
          )}
        </button>
      ...
      {activeTab === "parties" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-orange-50 hover:bg-orange-600 text-orange-700 hover:text-white border border-orange-200 dark:bg-orange-600/20 dark:hover:bg-orange-600 dark:text-orange-300 dark:hover:text-white dark:border-orange-500/30 rounded-xl text-xs font-bold flex items-center justify-center space-x-2 transition cursor-pointer"
              >
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-emerald-50 hover:bg-emerald-600 text-emerald-700 hover:text-white border border-emerald-200 dark:bg-emerald-600/20 dark:hover:bg-emerald-600 dark:text-emerald-300 dark:hover:text-white dark:border-emerald-500/30 rounded-xl text-xs font-bold flex items-center justify-center space-x-2 transition cursor-pointer"
              >
                <FileSpreadsheet className="w-3.5 h-3.5" />
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-xl text-xs font-bold flex items-center justify-center space-x-2 transition shadow-md shadow-emerald-600/20 cursor-pointer"
              >
                {importingParties ? (
                  <>
          ...
          {partySuccessCount > 0 && (
            <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/40 rounded-xl flex items-center justify-between text-xs">
              ...
              {partyErrorCount > 0 && (
                <span className="text-rose-600 dark:text-rose-400 font-semibold">{partyErrorCount} skipped due to invalid fields</span>
              )}
            </div>
          )}
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
              <div className="p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div className="font-bold text-slate-900 dark:text-white text-sm flex items-center space-x-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-400 font-mono font-medium">
              ...
              <div className="overflow-x-auto max-h-[400px]">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-100/80 dark:bg-slate-900 text-slate-700 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 text-[11px]">
                    <tr>
                      <th className="p-3">Type</th>
                      <th className="p-3">Legal Business Name</th>
                      <th className="p-3">Mobile</th>
                      <th className="p-3">GSTIN / PAN</th>
                      <th className="p-3 text-emerald-700 dark:text-emerald-400 font-bold">Drug License (20B/21B)</th>
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white dark:bg-slate-950">
                    {partyRows.map((row) => (
                      <tr key={row.rowId} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                        <td className="p-3">
                          <span
                        ...
                        <td className="p-3 font-semibold text-slate-900 dark:text-white">
                          ...
                          {row.tradeName !== row.legalName && (
                            <div className="text-[10px] text-slate-500 dark:text-slate-400">{row.tradeName}</div>
                          )}
                        </td>
                        <td className="p-3 font-mono text-slate-700 dark:text-slate-300">{row.mobile}</td>
                        <td className="p-3 font-mono text-xs">
                          {row.gstin ? (
                            <span className="text-emerald-700 dark:text-emerald-400 font-semibold">{row.gstin}</span>
                          ) : (
                            <span className="text-slate-400 dark:text-slate-500">Unregistered / B2C</span>
                          )}
                        </td>
                        <td className="p-3 font-mono text-xs text-emerald-700 dark:text-emerald-300">
                          {row.dl1 ? (
                            <div>
                              <span>{row.dl1}</span>
                              {row.dl2 && <span className="ml-1 text-slate-500 dark:text-slate-400">/ {row.dl2}</span>}
                            </div>
                          ) : (
                        ...
                        <td className="p-3 text-right font-mono font-bold">
                          {row.openingBal > 0 ? (
                            <span className={row.balanceType === 1 ? "text-orange-600 dark:text-orange-400" : "text-rose-600 dark:text-rose-400"}>
                        ...
                        <td className="p-3 text-center">
                          {row.isValid ? (
                            ...
                            <span
                              ...
                              className="px-2 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-400 font-semibold text-[10px]"
                            >
                              Invalid
                            </span>
                          )}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-orange-50 hover:bg-orange-600 text-orange-700 hover:text-white dark:bg-orange-600/20 dark:hover:bg-orange-600 dark:text-orange-300 dark:hover:text-white border border-orange-200 dark:border-orange-500/30 rounded-xl text-xs font-bold flex items-center justify-center space-x-2 transition"
              >
                <Download className="w-3.5 h-3.5" />
            ...
            <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 shadow-xs">
              ...
              <button
                ...
                className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-xl text-xs font-bold flex items-center justify-center space-x-2 transition shadow-md shadow-emerald-600/20"
              >
                {importingProducts ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
          ...
          {productSuccessCount > 0 && (
            <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/40 rounded-xl flex items-center justify-between text-xs">
              ...
              {productErrorCount > 0 && (
                <span className="text-rose-600 dark:text-rose-400 font-semibold">{productErrorCount} skipped</span>
              )}
            </div>
          )}
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white dark:bg-slate-950">
                    {productRows.map((row) => (
                      <tr key={row.rowId} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                        ...
                        <td className="p-3 font-mono text-slate-500 dark:text-slate-400">{row.sku}</td>
                        <td className="p-3 text-slate-700 dark:text-slate-300">{row.packing}</td>
                        <td className="p-3 font-mono text-slate-500 dark:text-slate-400">{row.hsn}</td>
                        <td className="p-3 text-center font-mono text-slate-700 dark:text-slate-300">{row.taxRate}%</td>
                        <td className="p-3 text-right font-mono text-slate-900 dark:text-white">₹{row.mrp.toFixed(2)}</td>
                        <td className="p-3 text-right font-mono text-amber-600 dark:text-amber-400">₹{row.purchaseRate.toFixed(2)}</td>
                        <td className="p-3 text-right font-mono text-emerald-600 dark:text-emerald-400 font-bold">₹{row.sellingPrice.toFixed(2)}</td>
                        <td className="p-3 text-right font-mono text-orange-600 dark:text-orange-400 font-bold">₹{(row.wholesaleRate || row.sellingPrice).toFixed(2)}</td>
                        <td className="p-3 font-mono text-xs">
                          <span className="text-orange-600 dark:text-orange-400">{row.batch}</span>
                          <span className="text-slate-500 dark:text-slate-400 ml-1">({row.expiry})</span>
                        </td>
      ...
      {activeTab === "history" && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
          <div className="p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 font-bold text-slate-900 dark:text-white text-sm">
            Onboarding Migration & Ingestion History
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-100/80 dark:bg-slate-900 text-slate-700 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 text-[11px]">
                <tr>
                  ...
                  <th className="p-3 text-right text-emerald-600 dark:text-emerald-400">Imported</th>
                  <th className="p-3 text-right text-rose-600 dark:text-rose-400">Errors</th>
                  <th className="p-3 text-center">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                {migrationLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    <td className="p-3 font-mono font-bold text-orange-600 dark:text-orange-400">{log.id}</td>
                    <td className="p-3 text-slate-500 dark:text-slate-400">{log.timestamp}</td>
                    <td className="p-3 font-semibold text-slate-900 dark:text-white">{log.type}</td>
                    <td className="p-3 text-slate-600 dark:text-slate-300">{log.source}</td>
                    <td className="p-3 text-right font-mono font-bold text-slate-900 dark:text-white">{log.total}</td>
                    <td className="p-3 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">{log.success}</td>
                    <td className="p-3 text-right font-mono font-bold text-rose-600 dark:text-rose-400">{log.failed}</td>
                    <td className="p-3 text-center">
                      <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 font-bold text-[10px] border border-emerald-200 dark:border-emerald-800/40">
                        COMPLETED
                      </span>
                    </td>
                  </tr>
