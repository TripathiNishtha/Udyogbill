  ...
  return (
    <div className="min-h-screen lg:h-screen w-screen overflow-y-auto lg:overflow-hidden bg-[#fff7ed] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-orange-100/60 via-[#fff7ed] to-amber-50/80 flex flex-col justify-between font-sans relative selection:bg-orange-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 flex-1 flex items-center justify-center min-h-0 relative z-10 py-4 sm:py-6">
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 xl:gap-12 items-center max-h-full">
          ...
          <div className="hidden lg:flex lg:col-span-5 flex-col justify-center space-y-5 text-slate-900 pr-2">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="p-3.5 rounded-2xl bg-white border border-slate-200/80 hover:border-orange-300 shadow-sm hover:shadow-md transition-all flex items-center gap-3.5 group">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900">Instant GST Auto-Fill</h4>
                  <p className="text-[11px] text-slate-500 font-medium">Auto-populates legal company name &amp; trade brand directly via GSTIN.</p>
                </div>
              </div>

              <div className="p-3.5 rounded-2xl bg-white border border-slate-200/80 hover:border-emerald-200 shadow-sm hover:shadow-md transition-all flex items-center gap-3.5 group">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900">Multi-Industry Architecture</h4>
                  <p className="text-[11px] text-slate-500 font-medium">Pharma FEFO, FMCG Schemes, Electronics Serials &amp; Garments Matrix.</p>
                </div>
              </div>

              <div className="p-3.5 rounded-2xl bg-white border border-slate-200/80 hover:border-amber-200 shadow-sm hover:shadow-md transition-all flex items-center gap-3.5 group">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900">100% Tax Compliant</h4>
                  <p className="text-[11px] text-slate-500 font-medium">Accurate CGST/SGST/IGST tax calculation, E-way bill &amp; QR receipts.</p>
                </div>
              </div>
            </div>

            {/* Trust Proof & Ratings Bar */}
            <div className="flex flex-wrap items-center gap-5 text-slate-600 text-xs pt-2 border-t border-slate-200">
              <div className="flex items-center gap-2">
                <span className="flex h-2.5 w-2.5 relative">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
                </span>
                <span className="font-bold text-slate-900">Trusted by 10,000+ businesses</span>
              </div>
              <div className="flex items-center gap-1.5 text-amber-500 font-bold">
                <span>★ ★ ★ ★ ★</span>
                <span className="text-slate-600 font-medium text-[11px]">4.9/5 Rating</span>
              </div>
            </div>
          </div>

          {/* RIGHT COLUMN: The Compact High-Contrast Light Registration Form Card */}
          <div className="lg:col-span-7 flex justify-center w-full">
            <div className="w-full max-w-xl bg-white rounded-3xl p-5 sm:p-7 shadow-xl shadow-slate-200/70 border border-slate-200 relative">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3.5">
                ...
                <div className="hidden sm:flex items-center gap-1.5 text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full font-bold">
              ...
              {formData.referralCode && (
                <div className="mb-3 py-1.5 px-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-emerald-800 text-xs font-medium">
                  <Gift className="w-4 h-4 text-emerald-600 shrink-0" />
                  <div>
                    <span className="font-bold">Partner Referral Code:</span>{" "}
                    <code className="font-mono bg-emerald-100 px-1.5 py-0.5 rounded font-bold text-emerald-900">
                      {formData.referralCode}
                    </code>
                  </div>
                </div>
              ...
              {success ? (
                <div className="py-8 px-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-center space-y-2.5">
                  <CheckCircle className="w-12 h-12 text-emerald-600 mx-auto animate-bounce" />
                  <h3 className="text-lg font-bold text-slate-900">Account Provisioned Successfully!</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  {error && (
                    <div className="py-2 px-3 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-2 text-rose-700 text-xs font-semibold animate-in fade-in">
                  ...
                  <div className="p-3 bg-orange-50/70 border border-orange-200/80 rounded-2xl space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-slate-500 hidden sm:inline font-medium">Auto-populates name &amp; brand</span>
                    </div>
                    ...
                    {gstAutoFilled && (
                      <div className="py-1.5 px-2.5 bg-emerald-50 border border-emerald-200 rounded-lg flex items-center gap-1.5 text-xs text-emerald-800 animate-in fade-in">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                        <span className="truncate">
                          <strong>Verified GSTIN:</strong> {gstAutoFilled.legalName}
                        </span>
                      </div>
                    )}

                    {gstError && (
                      <div className="py-1.5 px-2.5 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-1.5 text-xs text-amber-800 animate-in fade-in">
                        <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
                        <span className="truncate font-medium">{gstError}</span>
                      </div>
                    )}
                  </div>
      ...
      {/* Footer */}
      <footer className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-3 text-center text-xs text-slate-500 shrink-0 relative z-10 border-t border-slate-200/80 bg-white/60 backdrop-blur-sm">
