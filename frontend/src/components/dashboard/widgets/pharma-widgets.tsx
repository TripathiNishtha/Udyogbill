  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 shadow-xs space-y-2.5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xs sm:text-sm font-black text-slate-950 dark:text-white tracking-tight flex items-center space-x-1.5">
        ...
        <div className="flex items-center space-x-2">
          {batches.length > 0 && (
            <span className="text-[11px] font-mono font-bold px-2.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-300 dark:border-rose-500/20">
      ...
      {batches.length === 0 ? (
        <div className="p-6 text-center text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-950/40 rounded-lg border border-slate-200 dark:border-slate-800/60">
          {isHi
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
              {batches.slice(0, 5).map((b) => (
                <tr key={b.batchId} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-2.5 font-bold text-white">{b.batchNumber}</td>
                  <td className="p-2.5 font-sans font-medium text-slate-200 max-w-[140px] truncate" title={b.itemName}>
                    {b.itemName}
                  </td>
                  <td className="p-2.5 text-slate-400">{b.expiryDate?.slice(0, 10)}</td>
                  <td className="p-2.5 text-right font-bold text-white">
                    {b.currentStock} {b.uomName}
                  </td>
                  <td className="p-2.5 text-right text-slate-300">{formatCurrency(b.mrp)}</td>
                  <td className="p-2.5 text-right text-rose-400 font-bold">{formatCurrency(b.totalValueAtRisk)}</td>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
              {records.slice(0, 5).map((r, idx) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-2.5 font-bold text-white">{r.invoiceNumber}</td>
                  <td className="p-2.5 text-slate-400">{r.invoiceDate?.slice(0, 10)}</td>
                  <td className="p-2.5 font-sans">
                    <div className="font-semibold text-slate-200 max-w-[130px] truncate" title={r.doctorName}>
                      {r.doctorName || "Registered Medical Practitioner"}
                    </div>
                    {r.doctorRegistrationNumber && (
                      <div className="text-[10px] text-slate-500 font-mono">Reg: {r.doctorRegistrationNumber}</div>
                    )}
                  </td>
                  <td className="p-2.5 font-sans text-slate-300 max-w-[120px] truncate" title={r.patientName}>
                    {r.patientName}
                  </td>
                  <td className="p-2.5 font-sans font-medium text-indigo-300 max-w-[140px] truncate" title={r.drugName}>
                    {r.drugName}
                  </td>
                  <td className="p-2.5 text-slate-400">{r.batchNumber}</td>
