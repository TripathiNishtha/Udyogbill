...
export function QuickAddCustomerModal({ isOpen, onClose, onCustomerCreated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const licensePayload = formatLicensesForPayload(licenses);
  ...
  return (
    <div className="fixed inset-0 z-[200000] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/60 shrink-0">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Add New Customer Account</h3>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="p-5 space-y-3.5 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-200">Trade / Shop Name</label>
                <input
                  type="text"
              ...
              {/* GSTIN */}
              <div className="space-y-1 sm:col-span-2">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-200">Customer GSTIN</label>
                <input
                  type="text"
                  maxLength={15}
              ...
              {/* Business Licenses & Certifications (DL, FSSAI, ISO, IEC, etc.) */}
              ...
              {/* City */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-200">City</label>
                <input
                  type="text"
                  placeholder="City"
                  value={city}
              ...
              {/* State Code */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-200">State Code (GST)</label>
                <input
                  type="text"
                  maxLength={2}
                  placeholder="e.g. 27"
          ...
          <div className="p-3.5 sm:p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end space-x-2.5 bg-slate-50 dark:bg-slate-950/80 shrink-0">
            <button
              type="button"
              onClick={onClose}
