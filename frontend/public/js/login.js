...
$(document).ready(function() {
    ...
    if ($('#business_register_form').length) {
        ...
        form.steps({
            ...
            onFinished: function(event, currentIndex) {
                if ($('#email_otp_verified').val() === '1') {
                    form[0].submit();
                    return;
                }

                if (typeof sendRegistrationEmailOtp === 'function') {
                    sendRegistrationEmailOtp(false);
                }
            },
        });
    ...
    // Native file input on register (bootstrap-fileinput breaks the 2-col layout)
});
