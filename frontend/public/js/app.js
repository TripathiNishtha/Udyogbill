$(document).ready(function() {
    ...
    $(document).on('click', '.btn-modal', function(e) {
        ...
        var url = $(this).data('href') || $(this).attr('data-href');
                    ...
                    toastr.error(
                        xhr.responseJSON && xhr.responseJSON.message
                            ...
                            : (xhr.status === 403 || xhr.status === 401
                                ? 'Unauthorized'
                                : 'Could not load this page. Please refresh and try again.')
                    );
                }
            },
        });
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == 1 && result.receipt.html_content != '') {
                    ...
                    var previewed = false;
                    var receiptHtml = result.receipt.html_content || '';
                    var hasEwayJson = receiptHtml.indexOf('ub-eway-json-payload') !== -1
                        || receiptHtml.indexOf('ub-eway-json-actions') !== -1
                        || receiptHtml.indexOf('Download E-way JSON') !== -1
                        || receiptHtml.indexOf('E-Way JSON') !== -1
                        || receiptHtml.indexOf('ubDownloadEwayJson') !== -1;
                    if ((window.UB_PRINT_PREVIEW_ENABLED || hasEwayJson) && typeof window.ubOpenPrintPreview === 'function') {
                        previewed = window.ubOpenPrintPreview({
                            html: result.receipt.html_content,
...
/* Table dropdown clipping — see public/js/ub-dropdown-fix.js */
