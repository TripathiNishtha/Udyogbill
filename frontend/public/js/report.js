$(document).ready(function() {
    ...
    var stock_report_cols = [
        ...
        stock_report_cols.push({ data: 'total_sold_value', name: 'total_sold_value', searchable: false, orderable: false, defaultContent: '' });
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            for (var r in data){
                ...
                footer_total_sold_value += (data[r].total_sold_value && $(data[r].total_sold_value).data('orig-value')) ?
                    parseFloat($(data[r].total_sold_value).data('orig-value')) : 0;
            ...
            $('.footer_total_sold_value').html(__currency_trans_from_en(footer_total_sold_value));
    ...
    if ($('#sr_date_filter').length == 1) {
        function get_sr_dates() {
            var start = '';
            var end = '';
            if ($('input#sr_date_filter').length && $('input#sr_date_filter').val() && $('input#sr_date_filter').data('daterangepicker')) {
                start = $('input#sr_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                end = $('input#sr_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
            }
        ...
        //Sales representative report -> Total commission
        if ($('span#sr_total_commission').length > 0) {
        ...
        // Sales representative report -> Commission Payments Tab
        sr_commission_payments_table = $('table#sr_commission_payments_table').DataTable({
            processing: true,
            serverSide: true,
            fixedHeader: false,
            aaSorting: [[1, 'desc']],
            ...
            fnDrawCallback: function(oSettings) {
                if ($('#footer_commission_paid_total').length) {
                    $('#footer_commission_paid_total').text(
                        sum_table_col($('#sr_commission_payments_table'), 'paid-amount')
                    );
                }
                __currency_convert_recursively($('#sr_commission_payments_table'));
            },
        });

        // Sales representative report -> Agent Ledger Tab
        sr_commission_ledger_table = $('table#sr_commission_ledger_table').DataTable({
            processing: true,
            serverSide: true,
            fixedHeader: false,
            ordering: false,
            ajax: {
                url: '/commission-payments/ledger',
                data: function(d) {
                    var dates = get_sr_dates();
                    d.user_id = $('select#sr_id').val();
                    d.location_id = $('select#sr_business_id').val();
            ...
            fnDrawCallback: function(oSettings) {
                __currency_convert_recursively($('#sr_commission_ledger_table'));
            },
        });
        ...
        // Pay Commission Modal Trigger
        $(document).on('click', '#pay_commission_btn', function() {
            var user_id = $('select#sr_id').val();
            $.ajax({
                url: '/commission-payments/create' + (user_id ? '?user_id=' + user_id : ''),
                dataType: 'html',
                success: function(result) {
                    $('.commission_payment_modal')
                        .html(result)
                        .modal('show');
                    $('.commission_payment_modal .select2').select2();
                    $('#modal_commission_paid_on').datepicker({
                        autoclose: true,
        ...
        // Submit Pay Commission Form
        $(document).on('submit', '#pay_commission_form', function(e) {
            e.preventDefault();
            var form = $(this);
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success === true) {
                        $('.commission_payment_modal').modal('hide');
        ...
        // Edit Commission Payment Modal Trigger
        $(document).on('click', '.edit_commission_payment', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(result) {
                    $('.commission_payment_modal')
                        ...
                        .modal('show');
                    $('.commission_payment_modal .select2').select2();
                    $('#edit_commission_paid_on').datepicker({
                        autoclose: true,
                        format: datepicker_date_format,
                    });
                },
        ...
        // Submit Edit Commission Form
        $(document).on('submit', '#edit_commission_payment_form', function(e) {
            e.preventDefault();
            var form = $(this);
            var data = new FormData(this);
            $.ajax({
        ...
        $(document).on('click', '.delete_commission_payment', function(e) {
            e.preventDefault();
            var url = $(this).data('href');
            swal({
                title: LANG.sure,
        ...
        // View Commission Payment Voucher
        $(document).on('click', '.view_commission_payment', function(e) {
            e.preventDefault();
            var url = $(this).data('href');
            $.ajax({
                url: url,
        ...
        // Print Commission Ledger
        $(document).on('click', '#print_commission_ledger_btn', function() {
            $('#commission_ledger_printable_area').printThis();
        });
    }
...
function updateSalesRepresentativeReport() {
    ...
    if (typeof sr_commission_payments_table !== 'undefined') sr_commission_payments_table.ajax.reload();
    if (typeof sr_commission_ledger_table !== 'undefined') sr_commission_ledger_table.ajax.reload();
...
function salesRepresentativeTotalCommission() {
    ...
    $('span#sr_total_commission_paid').html(__fa_awesome());
    $('span#sr_total_commission_due').html(__fa_awesome());
    ...
    $.ajax({
        ...
        data: data_sell,
        success: function(data) {
            $('span#sr_total_commission').html(__currency_trans_from_en(data.total_commission, true));
            $('span#sr_total_commission_paid').html(__currency_trans_from_en(data.total_commission_paid, true));
            $('span#sr_total_commission_due').html(__currency_trans_from_en(data.total_commission_due, true));

            var calc_text = '';
            if (data.commission_percentage != 0) {
                if (data.total_sales_with_commission) {
                    calc_text = data.commission_percentage + '% of ' + __currency_trans_from_en(data.total_sales_with_commission);
                } else if (data.total_payment_with_commission) {
                    calc_text = data.commission_percentage + '% of ' + __currency_trans_from_en(data.total_payment_with_commission);
                }
            } else {
                calc_text = 'No % configured';
            }
            $('#sr_commission_calc_text').text(calc_text);
