...
$(document).ready(function() {
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        fetch_gst_transaction_suggestion(data.id);
    });

    set_default_customer();
    fetch_gst_transaction_suggestion($('#customer_id').val());
    ...
    // Ease Upgrade: Barcode-first fast scan mode + camera/voice helpers
    if ($('#search_product').length && $('#ease_scan_fast_toggle').length) {
        var easeScanFastMode = false;
        var easeLastScanCode = null;
        var easeLastScanAt = 0;
        var easeCameraInterval = null;
        var easeCameraStream = null;
        ...
        function easeIncrementLastRowQty() {
            var $lastQty = $('#pos_table tbody tr:last').find('input.pos_quantity');
            if (!$lastQty.length) {
                return false;
            }
            var qty = __read_number($lastQty) || 0;
            __write_number($lastQty, qty + 1);
            $lastQty.change();
            return true;
        }

        function easeHandleBarcodeScan(code) {
            var now = Date.now();
            if (easeLastScanCode === code && now - easeLastScanAt < 4000) {
                if (!easeIncrementLastRowQty()) {
                    pos_product_row(null, null, code);
                }
            } else {
                pos_product_row(null, null, code);
            }
            easeLastScanCode = code;
            easeLastScanAt = now;
            easeBeep();
        }

        function easeAttachScanListener() {
            ...
            onScan.attachTo(document, {
                ...
                onScan: function (sCode) {
                    ...
                    easeHandleBarcodeScan(sCode);
                },
                onScanError: function () {
                },
                minLength: 2,
                ignoreIfFocusOn: ['textarea']
            });
        }

        function easeSetScanMode(mode) {
            easeScanFastMode = mode;
            $('#ease_scan_fast_state').text(mode ? 'ON' : 'OFF');
            if (mode) {
                $('#search_product').focus();
                easeAttachScanListener();
            }
        }

        $('#ease_scan_fast_toggle').on('click', function () {
            easeSetScanMode(!easeScanFastMode);
        });

        $('#ease_mobile_scan_btn').on('click', function () {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia || typeof BarcodeDetector === 'undefined') {
                toastr.info('Camera barcode scan is not supported in this browser.');
                return;
            }
            ...
            var video = $('<video autoplay playsinline style="position:fixed;inset:10% 10%;z-index:9999;background:#000;width:80%;max-height:70%;"></video>');
            var closeBtn = $('<button type="button" style="position:fixed;right:12%;top:12%;z-index:10000;" class="btn btn-danger btn-sm">Close Scan</button>');
            $('body').append(video).append(closeBtn);

            navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }).then(function (stream) {
                easeCameraStream = stream;
                video[0].srcObject = easeCameraStream;
                var detector = new BarcodeDetector({ formats: ['code_128', 'ean_13', 'ean_8', 'upc_a', 'upc_e', 'qr_code'] });
                easeCameraInterval = setInterval(function () {
                    detector.detect(video[0]).then(function (barcodes) {
                        if (barcodes.length) {
                            easeHandleBarcodeScan(barcodes[0].rawValue);
                            closeBtn.click();
                        }
                    }).catch(function () {});
                }, 300);
            ...
            closeBtn.on('click', function () {
                if (easeCameraInterval) {
                    clearInterval(easeCameraInterval);
                    easeCameraInterval = null;
                }
                if (easeCameraStream) {
                    easeCameraStream.getTracks().forEach(function (t) { t.stop(); });
                    easeCameraStream = null;
                }
                video.remove();
                closeBtn.remove();
            });
        ...
        if ($('#ease_voice_search_btn').length) {
            $('#ease_voice_search_btn').on('click', function () {
                var SR = window.SpeechRecognition || window.webkitSpeechRecognition;
                if (!SR) {
                    toastr.info('Voice search is not supported in this browser.');
                    return;
                }
                var rec = new SR();
                rec.lang = 'en-IN';
                ...
                rec.onresult = function (event) {
                    ...
                    $('#search_product').val(text).focus().autocomplete('search');
                };
                rec.start();
            });
        }
...
$(document).on('change input', 'input[name="shipping_custom_field_2"]', function() {
    fetch_gst_transaction_suggestion($('#customer_id').val());
});

function ease_get_party_state_for_gst() {
    var shippingState = $('input[name="shipping_custom_field_2"]').val();
    if (shippingState && String(shippingState).trim() !== '') {
        return String(shippingState).trim();
    }

    return '';
}

function ease_is_direct_sell_screen() {
    return $('input[name="is_direct_sale"]').length > 0 &&
        $('input[name="is_direct_sale"]').val() == 1;
}

function apply_supply_tax_to_all_rows(gstResponse) {
    if (!ease_is_direct_sell_screen()) {
        return;
    }

    if (!gstResponse || !gstResponse.success) {
        return;
    }

    if (gstResponse.supply_type !== 'intra' && gstResponse.supply_type !== 'inter') {
        return;
    }

    var taxIds = [];
    $('#pos_table tbody tr').each(function() {
        var taxId = $(this).find('select.tax_id').val();
        if (taxId) {
            taxIds.push(taxId);
        }
    });
    ...
    $.ajax({
        method: 'POST',
        url: '/sells/pos/resolve-supply-tax-map',
        dataType: 'json',
        data: {
            _token: $('meta[name="csrf-token"]').attr('content'),
            contact_id: $('#customer_id').val(),
            shipping_state: ease_get_party_state_for_gst(),
            location_id: $('input#location_id').val(),
            tax_ids: taxIds,
        },
        success: function(result) {
            if (!result || !result.success || !result.tax_id_map) {
                return;
            }

            $('#pos_table tbody tr').each(function() {
                var row = $(this);
                var select = row.find('select.tax_id');
                var current = select.val();

                if (!current || !result.tax_id_map[current]) {
                    return;
                }

                var next = result.tax_id_map[current];
                if (next != current && select.find('option[value="' + next + '"]').length) {
...
function fetch_gst_transaction_suggestion(contact_id) {
    if (!ease_is_direct_sell_screen()) {
        return;
    }
    ...
    $.ajax({
        method: 'GET',
        url: '/sells/pos/gst-transaction-suggestion',
        dataType: 'json',
        data: {
        ...
        success: function(response) {
            if (response && response.success) {
                window.__ease_gst_supply_type = response.supply_type || 'unknown';

                if ($('#ease_gst_transaction_hint').length) {
                    $('#ease_gst_transaction_hint').text(response.hint || 'Manual tax mode');
                    if (response.business_state_code || response.party_state_code) {
                        var meta = 'Business: ' + (response.business_state_code || '-') + ', Party: ' + (response.party_state_code || '-');
                        $('#ease_gst_transaction_meta').text(meta);
                    } else {
                        $('#ease_gst_transaction_meta').text('');
                    }
                }

                apply_supply_tax_to_all_rows(response);
            } else if ($('#ease_gst_transaction_hint').length) {
                $('#ease_gst_transaction_hint').text('Manual tax mode');
                $('#ease_gst_transaction_meta').text('');
            }
        },
        error: function() {
...
function pos_should_merge_duplicate_product() {
    if ($('#pos_lot_batch_enabled').length && $('#pos_lot_batch_enabled').val() == '1') {
        return false;
    }
...
function pos_add_product_row_from_data(result) {
    if (result.success) {
        ...
        if (pos_should_merge_duplicate_product() && variation_id) {
            var is_added = false;
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (variation_id != null && pos_should_merge_duplicate_product()) {
        var is_added = false;
...
function calculate_billing_details(price_total) {
    ...
    var rounding_setting = $('#amount_rounding_method').val() ? parseFloat($('#amount_rounding_method').val()) : 0;
    var rounding_multiple = rounding_setting > 0 ? rounding_setting : 1;
    var round_off_data;
    if (rounding_multiple === 1) {
        var frac_part = total_payable - Math.floor(total_payable);
        var total_payable_rounded = frac_part > 0.5 ? Math.ceil(total_payable) : Math.floor(total_payable);
        round_off_data = { number: total_payable_rounded, diff: total_payable_rounded - total_payable };
    } else {
        round_off_data = __round(total_payable, rounding_multiple);
    }
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        var previewed = false;
        if (window.UB_PRINT_PREVIEW_ENABLED && typeof window.ubOpenPrintPreview === 'function') {
            previewed = window.ubOpenPrintPreview({
        ...
        if (!previewed) {
            ...
            var $tmp = $('<div>').html(receipt.html_content);
...
function update_shipping_address(data) {
    ...
    if ($('#shipping_custom_field_1').length) {
        var shippingDetails = data.shipping_custom_field_details;
        ...
        var shippingFieldValues = {
            shipping_custom_field_1: shippingDetails.shipping_custom_field_1 || data.country || '',
            shipping_custom_field_2: shippingDetails.shipping_custom_field_2 || data.state || '',
            shipping_custom_field_3: shippingDetails.shipping_custom_field_3 || data.city || '',
            shipping_custom_field_4: shippingDetails.shipping_custom_field_4 || data.zip_code || '',
            shipping_custom_field_5: shippingDetails.shipping_custom_field_5 || ''
        };

        $.each(shippingFieldValues, function(fieldName, fieldValue) {
    ...
    var shippingAddressValue = data.shipping_address || '';
    if (!shippingAddressValue) {
        shippingAddressValue = [data.address_line_1, data.address_line_2, data.city, data.state, data.country, data.zip_code]
            .filter(function(part) { return !!part; })
            .join(', ');
    }
    $('textarea[name="shipping_address"]').val(shippingAddressValue);
}
