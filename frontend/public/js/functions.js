...
function __cleanup_print_receipt() {
    setTimeout(function() {
...
/**
 * Silent direct print: load HTML in hidden iframe, auto-trigger print dialog.
 * No file download - user only selects printer and prints.
 * @param {string} htmlContent - Full HTML string for invoice/receipt
 * @returns {boolean} - true if initiated successfully, false to allow fallback
 */
function __silentPrintFromHtml(htmlContent) {
    if (typeof htmlContent !== 'string' || htmlContent.trim() === '') {
        return false;
    }
    if (window.__silentPrintInProgress) {
        return true;
    }
    try {
        window.__silentPrintInProgress = true;
        var iframe = document.createElement('iframe');
        iframe.style.cssText = 'position:absolute;width:0;height:0;border:none;visibility:hidden;';
        iframe.setAttribute('id', 'silent_print_iframe');
        ...
        function tryPrint() {
            ...
            setTimeout(function() {
                ...
                window.__silentPrintInProgress = false;
            }, 500);
        }
        ...
        if (imgCount === 0) {
            ...
            for (var i = 0; i < imgs.length; i++) {
                if (imgs[i].complete) {
                    ...
                    imgs[i].addEventListener('load', onImageDone);
                    imgs[i].addEventListener('error', onImageDone);
                }
            }
            setTimeout(function() {
                if (!didPrint && window.__silentPrintInProgress && iframe.parentNode) {
                    tryPrint();
                }
            }, 3000);
        }
        ...
        if (typeof console !== 'undefined' && console.error) {
            console.error('__silentPrintFromHtml error:', e);
        }
        return false;
    }
}
